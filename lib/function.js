...
const GIFBufferToVideoBuffer = async (image) => {
    ...
    await fs.writeFileSync(`./Assets/trash/${filename}.gif`, image)
    child_process.exec(
        `ffmpeg -i ./Assets/trash/${filename}.gif -movflags faststart -pix_fmt yuv420p -vf "scale=trunc(iw/2)*2:trunc(ih/2)*2" ./Assets/trash/${filename}.mp4`
    )
    await sleepy(4000)

    var buffer5 = await fs.readFileSync(`./Assets/trash/${filename}.mp4`)
    Promise.all([unlink(`./Assets/trash/${filename}.mp4`), unlink(`./Assets/trash/${filename}.gif`)])
    return buffer5
}
